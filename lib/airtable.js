...
/** Normaliza para comparar nombres de columnas de forma robusta */
...
/** Obtiene un campo del record por coincidencia flexible de nombres */
function getByNamesCI(record, candidates) {
  const wanted = candidates.map(normalizeKey);
  // Airtable SDK: record.fields es { "Nombre Columna": valor, ... }
  for (const [key, val] of Object.entries(record.fields || {})) {
    const nk = normalizeKey(key);
    if (wanted.includes(nk)) return val;
  }
  return undefined;
}
...
export async function listProviders({ campaigns, type, profession, specialty, subSpecialty } = {}) {
  ...
  const rows = records.map(r => {
    const nombre = asText(getByNamesCI(r, [
      "Nombre", "Nombre de proveedor", "Nombre del proveedor", "Nombre completo", "Proveedor", "Médico"
    ]));
    ...
    const campRaw = getByNamesCI(r, ["Campaña Asignada","Campañas","Campana Asignada","Campanas"]);
    ...
    const telefono = asText(getByNamesCI(r, [
      "Teléfono principal ","Telefono Secundario","Teléfono","Telefono","Teléfono principal","Telefono principal"
    ]));
    const email    = asText(getByNamesCI(r, ["Email","Correo","Correo electrónico","Correo electronico"]));
...
export async function getFacets(){
  // Pido todos los campos; luego hago matching flexible en cada fila.
  const records = await base(TABLE).select({ maxRecords: 3000 }).all();
  ...
  for (const r of records){
    ...
    const c = getByNamesCI(r, ["Campaña Asignada","Campañas","Campana Asignada","Campanas"]);
