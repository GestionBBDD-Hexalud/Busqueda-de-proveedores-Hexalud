...
/* ===== Parámetros ===== */
const PRIMARY_MAX_MIN = 60;    // “En la localidad”
const SECONDARY_MAX_MIN = 120; // Secundarias (≤ 2 hrs)
const PAGE_SIZE = 10;
...
async function copyFicha(r) {
  ...
  try { await navigator.clipboard.writeText(txt); alert("Ficha copiada ✔️"); }
  catch { alert("No se pudo copiar. Intenta desde un navegador compatible."); }
}
function buildFacetsFromResults(results) {
  ...
  for (const r of results || []) {
    ...
    if (tipo) setType.add(tipo); if (prof) setProf.add(prof); if (esp) setSpec.add(esp); if (sub) setSub.add(sub);
    camps.forEach((c)=>c && setCamp.add(c));
  }
  const sort = (a) => [...a].filter(Boolean).sort((x,y)=>x.localeCompare(y,"es"));
  return { types:sort(setType), professions:sort(setProf), specialties:sort(setSpec), subSpecialties:sort(setSub), campaigns:sort(setCamp) };
}
...
export default function Home() {
  ...
  useEffect(()=>{(async()=>{try{const {data}=await axios.get("/api/facets");setFacets(data||{});}catch{}})();},[]);

  /* Fallback facets desde resultados */
  ...
  function resetFilters(){
    setType("");setProfession("");setSpecialty("");setSubSpecialty("");setCampaigns([]);
  ...
  async function search(){
    try{
      ...
      setResults(data.results||[]); setOrigin(data.origin||null);
      if(!data.results?.length) setError("No hay opciones con los filtros seleccionados.");

      if(!facets.types?.length && data.results?.length){
        const d=buildFacetsFromResults(data.results);
        setFacets(prev=>({...prev,
          types:prev.types?.length?prev.types:d.types,
          professions:prev.professions?.length?prev.professions:d.professions,
          specialties:prev.specialties?.length?prev.specialties:d.specialties,
          subSpecialties:prev.subSpecialties?.length?prev.subSpecialties:d.subSpecialties,
          campaigns:prev.campaigns?.length?prev.campaigns:d.campaigns
        }));
      }
  ...
  const groupBySpec = (arr)=>arr.reduce((acc,r)=>{const k=(r.especialidad||r["Especialidad"]||"Otros").trim()||"Otros";(acc[k]??=[]).push(r);return acc;},{});  
  const primaryVisibleBySpec   = useMemo(()=>groupBySpec(primaryVisible),[primaryVisible]);
  const secondaryVisibleBySpec = useMemo(()=>groupBySpec(secondaryVisible),[secondaryVisible]);
  ...
  useEffect(()=>{
    ...
    suggestTimeout.current=setTimeout(async()=>{
      try{
        ...
        const opts=(data.features||[]).map(f=>({id:f.id,label:f.place_name,lng:f.center?.[0],lat:f.center?.[1]}));
        setSuggestions(opts); setShowSuggest(true);
    ...
    (async()=>{
      if(!mapboxRef.current){
        ...
        mapRef.current=new mapboxgl.Map({
          container:"hexalud-map", style:"mapbox://styles/mapbox/streets-v11",
          zoom:12, center: origin? [origin.lng,origin.lat] : [-99.1332,19.4326]
        });
      ...
      // Origen (👤)
      if(origin){
        const el=document.createElement("div"); el.style.fontSize="20px"; el.textContent="👤";
        const mk=new mbx.Marker({element:el}).setLngLat([origin.lng,origin.lat])
          .setPopup(new mbx.Popup().setText("Paciente (origen)")).addTo(map);
        markersRef.current.push(mk); map.setCenter([origin.lng,origin.lat]);
      }
      // Proveedores (🏥)
      for(const r of results.slice(0,100)){
        ...
        const el=document.createElement("div"); el.style.fontSize="20px"; el.textContent="🏥";
        const mk=new mbx.Marker({element:el}).setLngLat([lng,lat]).setPopup(
  ...
  async function drawRouteTo(r){
    ...
    try{
      ...
      const route=data?.routes?.[0]; if(!route?.geometry) return;
      const geojson={type:"FeatureCollection",features:[{type:"Feature",geometry:route.geometry,properties:{}}]};

      if(map.getSource(routeSrcId.current)){ if(map.getLayer(routeLayerId.current)) map.removeLayer(routeLayerId.current); map.removeSource(routeSrcId.current); }
      map.addSource(routeSrcId.current,{type:"geojson",data:geojson});
      map.addLayer({id:routeLayerId.current,type:"line",source:routeSrcId.current,paint:{"line-width":5,"line-color":"#0ea5e9"}});
    ...
    <main style={{fontFamily:"system-ui",padding:20,maxWidth:1300,margin:"0 auto"}}>
      ...
      <div style={{background:"#fff",padding:16,borderRadius:12,boxShadow:"0 2px 12px rgba(0,0,0,.06)",marginTop:12,position:"relative"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:10}}>
          ...
          <div>
            ...
            <select value={profession} onChange={(e)=>setProfession(e.target.value)} disabled={disabledProfession}
              style={{width:"100%",padding:10,borderRadius:10,border:"1px solid #ddd",background:disabledProfession?"#f3f4f6":"#fff"}}
          ...
          <div>
            ...
            <select value={specialty} onChange={(e)=>setSpecialty(e.target.value)} disabled={disabledSpecialty}
          ...
          <div>
            ...
            <select value={subSpecialty} onChange={(e)=>setSubSpecialty(e.target.value)} disabled={disabledSubSpec}
        ...
        <div style={{marginTop:12,position:"relative"}}>
          ...
          <input value={address} onChange={(e)=>setAddress(e.target.value)} onFocus={()=>{if(suggestions.length) setShowSuggest(true);}}
          ...
          {showSuggest && suggestions.length>0 && (
            <div style={{position:"absolute",top:"100%",left:0,right:0,background:"#fff",border:"1px solid #ddd",borderRadius:8,marginTop:4,zIndex:10,maxHeight:220,overflowY:"auto"}}>
              {suggestions.map(s=>(
                <div key={s.id} onMouseDown={()=>selectSuggestion(s)} style={{padding:10,cursor:"pointer"}}>{s.label}</div>
              ))}
              <div style={{padding:8,fontSize:12,color:"#666",borderTop:"1px solid #eee"}}>Autocompletado por Mapbox</div>
            </div>
          )}
        </div>
        ...
        <div style={{marginTop:10,display:"flex",gap:8,flexWrap:"wrap"}}>
          {campaignOptions.map(t=>(
            <button key={t} onClick={()=>setCampaigns(p=>p.includes(t)?p.filter(x=>x!==t):[...p,t])}
              style={{padding:"6px 10px",borderRadius:999,border:"1px solid #ddd",background:campaigns.includes(t)?"#0ea5e9":"#fff",color:campaigns.includes(t)?"#fff":"#111"}}>{t}</button>
          ))}
        </div>
        ...
        <div style={{marginTop:12,display:"flex",gap:10,flexWrap:"wrap"}}>
          ...
          <button onClick={()=>setShowMap(s=>!s)} disabled={!results.length}
      ...
      {!hasPrimary && results.length>0 && (
        <div style={{marginTop:12,padding:12,borderRadius:10,background:"#fff3f3",border:"1px solid #ffd6d6",color:"#991b1b"}}>
          <strong>Sin proveedor en la localidad</strong><br/>Mostramos opciones secundarias con propuestas a no más de 2 hrs de distancia.
        </div>
      )}
      ...
      <div style={{display:"grid",gridTemplateColumns:showMap?"1.1fr 0.9fr":"1fr",gap:16,marginTop:16}}>
        ...
        <div style={{position:"relative",zIndex:2}}>
          ...
          {primaryVisible.length>0 && (
            <div style={{marginBottom:16}}>
              ...
              {Object.entries(primaryVisibleBySpec).sort((a,b)=>a[0].localeCompare(b[0],'es')).map(([spec,items])=>(
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",gap:16}}>
                          <div style={{display:"flex",gap:10}}>
                            ...
                            <div>
                              ...
                              <div style={{fontSize:14,color:"#555"}}>{(r.direccion||r.Dirección||"")} · {(r.municipio||"")}{(r.estado?`, ${r.estado}`:"")}</div>
                              ...
                              <div style={{marginTop:8,display:"flex",gap:8,flexWrap:"wrap"}}>
                                <button onClick={()=>copyFicha(r)} style={{border:"1px solid #ddd",background:"#fff",borderRadius:8,padding:"6px 8px"}}>Copiar ficha</button>
                                <button onClick={()=>drawRouteTo(r)} style={{border:"1px solid #ddd",background:"#fff",borderRadius:8,padding:"6px 8px"}}>Ver en mapa</button>
                              </div>
                            </div>
                          </div>
              ...
              {primaryListAll.length>primaryLimit && (
                <div style={{display:"flex",justifyContent:"center",marginTop:8}}>
                  <button onClick={()=>setPrimaryLimit(n=>n+PAGE_SIZE)} style={{padding:"8px 12px",borderRadius:10,border:"1px solid #ddd",background:"#fff"}}>Ver más</button>
                </div>
              )}
            </div>
          )}
          ...
          {secondaryVisible.length>0 && (
            <div style={{marginTop:8}}>
              <div style={{fontWeight:700,fontSize:16,color:"#0f766e",margin:"6px 0"}}>Opciones secundarias (≤ {SECONDARY_MAX_MIN/60} hrs)</div>
              {Object.entries(secondaryVisibleBySpec).sort((a,b)=>a[0].localeCompare(b[0],'es')).map(([spec,items])=>(
                <div key={spec} style={{marginBottom:12}}>
                  <div style={{fontWeight:700,color:"#0f766e",margin:"6px 0"}}>{spec}</div>
                  <ul style={{display:"grid",gap:10}}>
                    {items.map(r=>(
              ...
              {secondaryListAll.length>secondaryLimit && (
                <div style={{display:"flex",justifyContent:"center",marginTop:8}}>
                  <button onClick={()=>setSecondaryLimit(n=>n+PAGE_SIZE)} style={{padding:"8px 12px",borderRadius:10,border:"1px solid #ddd",background:"#fff"}}>Ver más</button>
                </div>
              )}
            </div>
          )}
        ...
        {showMap && (
          <div style={{position:"relative",zIndex:1}}>
            {activeId && (()=>{const r=results.find(x=>x.id===activeId); if(!r) return null;
              return (<div style={{marginBottom:8,background:"#fff",padding:10,borderRadius:10,border:"1px solid #eee"}}>
