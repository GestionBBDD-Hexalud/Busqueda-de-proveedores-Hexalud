...
/* Descripción inteligente SIEMPRE mostrando algo útil */
function describeLine(r) {
  ...
  // Construye con lo que haya disponible
  const parts = [];
  ...
  // Si no hay nada, por lo menos devuelve el tipo o “(sin detalle)”
...
export default function Home() {
  ...
  // 1) Intentamos traer facets del backend
  useEffect(() => {
    (async () => {
      try {
        ...
        // Silencio: habrá fallback con resultados
      }
    })();
  }, []);

  // 2) Opciones VISIBLES: si backend viene vacío, usamos derivadas de resultados
  ...
  async function search() {
    try {
      ...
      // Relleno facets si backend vino vacío
  ...
  // Agrupar por especialidad (o “Otros” si no hay)
  ...
  // Lógica de habilitado: los selects SIEMPRE se muestran; se deshabilitan si no aplican
